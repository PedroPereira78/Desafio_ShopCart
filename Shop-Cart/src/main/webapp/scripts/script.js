...
modelsJson.map((item, produtos)=>{
    ...
    modelsItem.setAttribute('data-key', produtos);
    ...
    modelsItem.querySelector('a').addEventListener('click', (e)=>{
        ...
        cs('.modelsInfo--size').forEach((size, sizeIndex)=>{
            if(sizeIndex == 2) {
                ...
                c('.modelsInfo--actualPrice').innerHTML = `€ ${modelsJson[key].price[sizeIndex].toFixed(2)}`;
            }
